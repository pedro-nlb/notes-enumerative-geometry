...
\begin{cor}[B\'{e}zout]
    If $X_{1},\ldots,X_{k}\subseteq \mathbb{P}^{n}$ are subvarieties of codimensions $c_{1},\ldots,c_{k}$ respectively, with $\sum c_{i}\leqslant n$ and intersecting generically transversely, then
...
\begin{defn}
    For $k\leqslant \dim(V)=n$, the \textit{Grsassmannian} $G(k,V)$ is defined as the set of $k$-dimensional vector subspaces of $V$ together with the projective variety structure induced by the Pl\"{u}cker embedding:
    \begin{align*}
    ...
    \[ \{[\eta]\mid \operatorname{rk}(V\xrightarrow{(-)\wedge\eta}\wedge^{k+1}V)\leqslant n-k\}, \]
...
\begin{thm}
    We have
    \[ A:=A(\mathbb{G}(1,3))=\oplus_{0\leqslant b\leqslant a \leqslant 2}\mathbb{Z} \sigma_{a,b} \]
